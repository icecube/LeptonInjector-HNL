 ...
 * Adapted to include HNL-specific kinematic conditions
 *
...
namespace
{
     ...
     * Added: HNL-specific condition from HEP PH 0407371 Eq. 7
     *
     ...
     */
    bool kinematicallyAllowed(double mHNL, double x, double y, double E, double M, double m)
    {
        //Added conditions:
//         double proton_mass = 0.938272;
//         double neutron_mass = 0.939565;
//         double mI = (proton_mass + neutron_mass)/2;
//         double d_lepton = mHNL/(2*mI*E);
        
//         double h = x*y*d_lepton;   
//         if ((pow(h, 2)*(1 + x*d_lepton) - h*x*d_lepton + x*d_lepton > 0 )){
//             return false;
//         }
        double tau_mass = 1.777;
        double s = pow(M,2) + 2*M*E;
        double d_N = pow(M,2)/(s-pow(M,2));
        double d_tau = pow(mHNL,2)/(s-pow(M,2));//tau_mass**2/(s-M**2);
        
        double h = x*y + d_tau;   
        if ((pow(h, 2)*(1 + x*d_N) - (x + d_tau)*h + x*d_tau > 0 )){
            return false;
        }
...
HNLCrossSection::finalStateRecord
HNLCrossSection::sampleFinalState(double HNL_mass, double energy,
    ...
    if (interaction_ ==1 || interaction_==2){
        return( HNLCrossSection::sampleFinalState_DIS(HNL_mass, energy, scatteredType, random ) );
    }else if(interaction_==3){
        return( HNLCrossSection::sampleFinalState_GR(HNL_mass,  energy, scatteredType, random ) );
...
HNLCrossSection::finalStateRecord HNLCrossSection::sampleFinalState_DIS(double HNL_mass, double energy,
                               I3Particle::ParticleType scatteredType,
                               boost::shared_ptr<I3RandomService> random) const{
	...
	do{
		...
		}while(trialQ<Q2Min_ || !kinematicallyAllowed(HNL_mass, pow(10.,kin_vars[1]),pow(10.,kin_vars[2]),energy,targetMass_,m));
	...
	for(size_t j=0; j<=burnin; j++){
        ...
        while(trialQ<Q2Min_ || !kinematicallyAllowed(HNL_mass,
...
HNLCrossSection::finalStateRecord
HNLCrossSection::sampleFinalState_GR(double HNL_mass, double energy,
                                I3Particle::ParticleType scatteredType,
                                boost::shared_ptr<I3RandomService> random) const{
...
double HNLCrossSection::evaluateCrossSection(double HNL_mass, double energy, double x, double y,
	...
	if(!kinematicallyAllowed(HNL_mass, x, y, energy, targetMass_, particleMass(scatteredType)))
		return 0;
...
double HNLCrossSection::evaluateTotalCrossSection(double energy) const
{
    double log_energy = std::log10(energy);
...
double HNLCrossSection::getMinimumEnergy() const
{
    return std::pow(10., totalCrossSection_->lower_extent(0));
}

double HNLCrossSection::getMaximumEnergy() const
{
