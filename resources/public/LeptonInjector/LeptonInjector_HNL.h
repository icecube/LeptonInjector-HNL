#ifndef LEPTONINJECTOR_HNL_H_INCLUDED
#define LEPTONINJECTOR_HNL_H_INCLUDED
...
namespace LeptonInjector_HNL{
	...
	struct BasicInjectionConfiguration : public I3FrameObject{
		...
		//custom verticle center of the cylinder
		double cylinderCenter;
	...
	struct BasicEventProperties : public I3FrameObject{
		...
		//energy of outgoing neutrino
		double outgoingNeutrinoEnergy;

		//is it a physical procesS?
		bool physical;

		double decay_channel;

		friend class icecube::serialization::access;
		template <typename Archive>
		void serialize(Archive&, unsigned);
	};
	//In the old LI-HNL, there's a huge commented block of some oss streamt hing to save event properties - not sure what its purpose was.
