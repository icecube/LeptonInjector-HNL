...
const std::string defaultCrosssectionPath=std::string(getenv("I3_SRC"))+"/LeptonInjector_HNL/resources/test_xs.fits";
const std::string defaultTotalCrosssectionPath=std::string(getenv("I3_SRC"))+"/LeptonInjector_HNL/resources/test_xs_total.fits";
...
struct service_initializer{
	service_initializer(){
		...
		try{
			boost::python::import("icecube.LeptonInjector_HNL");
...
void ConfigureStandardParams(I3Module& inj){
	...
	if(inj.GetConfiguration().Has("Generators")){ //MultiInjector
		using LeptonInjector_HNL::MinimalInjectionConfiguration;
...
boost::shared_ptr<LeptonInjector_HNL::OutputCollector> connectCollector(I3Module& mod){
	using LeptonInjector_HNL::OutputCollector;
